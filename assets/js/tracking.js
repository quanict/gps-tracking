...
var tracking = {opacity:1.0,weight:4,trackLink:'',center:false,
	...
	nodeItem:function($type,$val){
		$unit = '';
		if($type == 'vaq'){
			$unit = '<span class="unit" >V</span>';
		} else if($type=='speed'){
			$unit = '<span class="unit" >Km/h</span>';
		}
		return '<div class="item" ><div class="icon-h16 icon-h16-'+$type+'" ></div><span class="value" name="'+$type+'">'+$val+'</span>'+$unit+'</div>';
	},
	nodeInfo:function($node){
		$html = tracking.nodeItem('speed',$node.speed)
				...
				+tracking.nodeItem('address','')
				+tracking.nodeItem('time',i18n.last_update+' '+vmap.date($node.t*1000,'H:i:s (d/m/Y)') )
				;
		$('#vehicle-info').html($html);
		vmap.bind_geocode($node.la,$node.lo,$('#vehicle-info span[name=address]'));
//		repo.geocoder =  new google.maps.Geocoder();
//		repo.geocoder.geocode({'latLng': vmap.LatLng($node.la,$node.lo)}, function(results, status) {
//	        if (status == google.maps.GeocoderStatus.OK) {
//	        	if (results[1]) {
//	        		$('#vehicle-info span[name=address]').html(results[1].formatted_address);
//	            } 
//	        } 
//        });
	},
			  ...
			  success: function(data){
				  if(data && data != '' && data[0] ) {
					...
					if($itemSize > 1){
						...
						// show true info when checking one vechanel
